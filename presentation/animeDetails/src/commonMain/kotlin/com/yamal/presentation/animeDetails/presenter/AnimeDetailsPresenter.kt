package com.yamal.presentation.animeDetails.presenter
...
class AnimeDetailsPresenter(private val animeRepository: AnimeRepository, private val animeId: Int) : Presenter<AnimeDetailsUi, AnimeDetailsUi, AnimeDetailsIntent, Nothing>() {

    private fun getAnimeDetails() {
        updateInternalState { it.copy(
            loading = true
        ) }
    ...
    override fun processIntent(intent: AnimeDetailsIntent) {
        when(intent) {
            AnimeDetailsIntent.Refresh -> {getAnimeDetails()}
        }
    }
}
